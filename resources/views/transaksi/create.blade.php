...
<div class="max-w-6xl mx-auto">
    ...
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-white">Buat Akad Pinjaman (Rahn)</h2>
        <p class="text-sm text-slate-400 mt-1">Lengkapi data akad. Setelah disimpan, akad akan berstatus <span class="text-amber-400 font-semibold">DRAFT</span> dan harus dikirim ke admin untuk verifikasi.</p>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="glass-card p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            <label class="block text-xs font-medium text-slate-400 mb-1">Pilih Barang Jaminan <span class="text-rose-400">*</span></label>
                            <div id="barangContainer" class="space-y-3">
                                <p class="text-sm text-slate-500 italic">Pilih nasabah terlebih dahulu...</p>
                            </div>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-400 mb-1">Tanggal Transaksi</label>
                            <input type="date" name="tanggal_transaksi" value="{{ date('Y-m-d') }}" class="w-full glass bg-white/5 border-white/10 rounded-lg px-3 py-2.5 text-white text-sm" required>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-400 mb-1">Tenor (Hari)</label>
                            <input type="number" name="tenor_hari" id="tenorSelect" value="30" class="w-full glass bg-white/5 border-white/10 rounded-lg px-3 py-2.5 text-slate-400 text-sm text-center font-bold cursor-not-allowed" readonly>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-400 mb-1">Metode Pembayaran Biaya</label>
                            <select name="metode_pembayaran" id="metodePembayaran" class="w-full glass bg-white/5 border-white/10 rounded-lg px-3 py-2.5 text-white text-sm" required>
                                <option value="potong_pinjaman" class="bg-slate-800">Dipotong dari Pinjaman</option>
                                <option value="bayar_dimuka" class="bg-slate-800">Dibayar di Awal (Cash)</option>
                            </select>
            ...
            <div>
                <div class="glass-card p-6 border-t-4 border-sky-500 sticky top-8">
                    <h3 class="text-base font-semibold text-white mb-5">Ringkasan Pinjaman</h3>
                    <div class="space-y-3 text-sm">
                        <div class="flex justify-between"><span class="text-slate-400">Total Taksiran</span><span class="text-white font-mono" id="sumTaksiran">Rp 0</span></div>
                        <div class="flex justify-between"><span class="text-slate-400">Plafon Pinjaman (QARD)</span><span class="text-emerald-400 font-mono font-bold" id="sumPinjaman">Rp 0</span></div>
                        <div class="flex justify-between border-t border-white/5 pt-3"><span class="text-slate-400">Ijarah / 30 Hari</span><span class="text-indigo-400 font-mono" id="sumUjrah">Rp 0</span></div>
                        <div class="flex justify-between"><span class="text-slate-400 text-xs">Ijarah = <span id="ijarahPct">{{ $settings['ijarah_persen'] ?? 2 }}</span>% × Taksiran</span></div>
                        <div class="flex justify-between"><span class="text-slate-400">Biaya Admin</span><span class="text-white font-mono" id="sumBiayaAdmin">Rp 0</span></div>
                        <div class="flex justify-between border-t border-white/5 pt-3"><span class="text-slate-400">Total Biaya</span><span class="text-rose-400 font-mono" id="sumTotalBiaya">Rp 0</span></div>
                        <div class="flex justify-between"><span class="text-slate-400">Metode</span><span class="text-xs text-amber-400 font-medium" id="sumMetode">Potong Pinjaman</span></div>
                        <div class="flex justify-between border-t-2 border-sky-500/30 pt-3"><span class="text-white font-bold">Diterima Nasabah</span><span class="text-lg text-emerald-400 font-mono font-bold" id="sumDiterima">Rp 0</span></div>
                    </div>
                    <div class="mt-6">
                        <button type="submit" class="btn-gradient w-full py-3 rounded-xl text-sm font-semibold">Simpan Draft Akad</button>
    ...
    barangs.forEach(b => {
        ...
        div.innerHTML = `
            <div class="flex items-center cursor-pointer" onclick="toggleBarang(this)">
                <input type="radio" name="barang_id" value="${b.id}" data-taksiran="${b.taksiran}" data-kategori="${b.kategori}" data-max-pinjaman="${maxP}" class="barang-radio w-5 h-5 border-white/10 bg-slate-800 text-sky-500 focus:ring-sky-500 mr-4">
                <div class="flex-1">
                    <p class="text-sm font-medium text-white">${b.nama_barang}</p>
            ...
            <div class="pinjaman-input-wrapper mt-3 hidden">
                <label class="block text-xs text-slate-400 mb-1">Jumlah Pinjaman (Max: ${fmt.format(maxP)})</label>
                <input type="text" name="pinjaman_items[${b.id}]" value="${maxP}" data-max="${maxP}" class="currency-input w-full glass bg-white/5 border-white/10 rounded-lg px-3 py-2 text-white text-sm" oninput="validateAndSummary(this, ${maxP})">
