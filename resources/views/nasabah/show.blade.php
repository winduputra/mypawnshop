    ...
    <div class="max-w-4xl mx-auto">
        <div class="mb-6 flex justify-between items-center">
            ...
            <div class="flex gap-3">
                <a href="{{ route('nasabah.edit', $nasabah) }}" class="bg-white border border-slate-200 px-4 py-2 rounded-xl text-sm text-sky-400 hover:bg-white/10 flex items-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-2 space-y-6">
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-8">
                    <h3 class="text-xl font-bold text-slate-800 mb-6">Informasi Nasabah</h3>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <p class="text-lg font-medium text-slate-800 font-mono">{{ $nasabah->nik }}</p>
                        </div>
                        <div class="sm:col-span-2">
                            <p class="text-xs text-slate-500 uppercase font-semibold">Nama Lengkap</p>
                            <p class="text-lg font-medium text-slate-800">{{ $nasabah->nama }}</p>
                        </div>
                        <div class="sm:col-span-2">
                            <p class="text-xs text-slate-500 uppercase font-semibold">Alamat Lengkap (KTP)</p>
                            <p class="text-base text-slate-800 mt-1">{{ $nasabah->alamat }}</p>
                        </div>
                        <div class="sm:col-span-2">
                            <p class="text-xs text-slate-500 uppercase font-semibold">Alamat Domisili</p>
                            <p class="text-base text-slate-800 mt-1">{{ $nasabah->alamat_domisili ?: 'Sama dengan KTP' }}</p>
                        </div>
                    </div>
                </div>

                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-8">
                    <h3 class="text-lg font-bold text-slate-800 mb-5">Kontak & Data Pribadi</h3>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                        ...
                        <div>
                            ...
                            <p class="text-lg font-medium text-slate-800">{{ $nasabah->email ?: '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-slate-500 uppercase font-semibold">Nama Ibu Kandung</p>
                            <p class="text-lg font-medium text-slate-800">{{ $nasabah->nama_ibu_kandung ?: '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-slate-500 uppercase font-semibold">Pekerjaan</p>
                            <p class="text-lg font-medium text-slate-800">{{ $nasabah->pekerjaan ?: '-' }}</p>
                        </div>
                        <div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-8">
                    <h3 class="text-lg font-bold text-slate-800 mb-5">Informasi Rekening</h3>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        <div>
                            <p class="text-xs text-slate-500 uppercase font-semibold">Nama Bank</p>
                            <p class="text-lg font-medium text-slate-800">{{ $nasabah->nama_bank ?: '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-slate-500 uppercase font-semibold">No. Rekening</p>
                            <p class="text-lg font-medium text-slate-800 font-mono">{{ $nasabah->no_rekening ?: '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-slate-500 uppercase font-semibold">Nama Pemilik Rekening</p>
                            <p class="text-lg font-medium text-slate-800">{{ $nasabah->nama_pemilik_rekening ?: '-' }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 text-center">
                    ...
                    @if($nasabah->foto_ktp)
                        <a href="{{ asset('storage/' . $nasabah->foto_ktp) }}" target="_blank" class="block">
                            <img src="{{ asset('storage/' . $nasabah->foto_ktp) }}" alt="Foto KTP" class="w-full rounded-xl border-2 border-slate-300 shadow-lg hover:opacity-80 transition">
                        </a>
                        <p class="text-[10px] text-slate-500 mt-2">Klik gambar untuk memperbesar</p>
                    @else
                        <div class="w-full aspect-video rounded-xl bg-white border-2 border-dashed border-white/20 flex flex-col items-center justify-center text-slate-500 mx-auto">
