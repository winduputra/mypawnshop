...
<div class="max-w-6xl mx-auto">
    ...
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-slate-800">Registrasi Nasabah Baru</h2>
        <p class="text-sm text-slate-500 mt-1">Lengkapi data di bawah ini untuk mendaftarkan nasabah.</p>
    </div>
    <form action="{{ route('nasabah.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-xs font-medium text-slate-500 mb-1">ID Nasabah (Otomatis)</label>
                        <input type="text" value="NSB-{{ str_pad($nextId, 5, '0', STR_PAD_LEFT) }}" class="w-full bg-white border border-slate-200 bg-white border-slate-300 rounded-lg px-3 py-2.5 text-slate-800 font-mono text-sm" disabled>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-slate-500 mb-1">Nama Lengkap <span class="text-rose-400">*</span></label>
                        <input type="text" name="nama" value="{{ old('nama') }}" class="w-full bg-white border border-slate-200 bg-white border-slate-300 rounded-lg px-3 py-2.5 text-slate-800 text-sm focus:border-sky-500 focus:ring-sky-500" required placeholder="Masukkan nama sesuai KTP">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-slate-500 mb-1">NIK (16 Digit) <span class="text-rose-400">*</span></label>
                        <input type="text" name="nik" value="{{ old('nik') }}" class="w-full bg-white border border-slate-200 bg-white border-slate-300 rounded-lg px-3 py-2.5 text-slate-800 text-sm focus:border-sky-500 focus:ring-sky-500" required maxlength="16" placeholder="3201xxxxxxxxxxxx">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="pekerjaan" value="{{ old('pekerjaan') }}" class="w-full bg-white border border-slate-200 bg-white border-slate-300 rounded-lg px-3 py-2.5 text-slate-800 text-sm focus:border-sky-500 focus:ring-sky-500" required placeholder="Contoh: PNS, Swasta">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-500 mb-1">Nama Ibu Kandung <span class="text-rose-400">*</span></label>
                            <input type="text" name="nama_ibu_kandung" value="{{ old('nama_ibu_kandung') }}" class="w-full bg-white border border-slate-200 bg-white border-slate-300 rounded-lg px-3 py-2.5 text-slate-800 text-sm focus:border-sky-500 focus:ring-sky-500" required>
                        </div>
                    </div>
                    <div>
                        ...
                        <select name="status_pernikahan" class="w-full bg-white border border-slate-200 bg-white border-slate-300 rounded-lg px-3 py-2.5 text-slate-800 text-sm focus:border-sky-500 focus:ring-sky-500" required>
                            <option value="" disabled {{ !old('status_pernikahan') ? 'selected' : '' }} class="bg-white">Pilih...</option>
                            <option value="Menikah" {{ old('status_pernikahan') == 'Menikah' ? 'selected' : '' }} class="bg-white">Menikah</option>
                            <option value="Belum Menikah" {{ old('status_pernikahan') == 'Belum Menikah' ? 'selected' : '' }} class="bg-white">Belum Menikah</option>
                            <option value="Duda/Janda" {{ old('status_pernikahan') == 'Duda/Janda' ? 'selected' : '' }} class="bg-white">Duda/Janda</option>
                        </select>
                    </div>
                    <div>
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="telepon" value="{{ old('telepon') }}" class="w-full bg-white border border-slate-200 bg-white border-slate-300 rounded-lg px-3 py-2.5 text-slate-800 text-sm focus:border-sky-500 focus:ring-sky-500" required placeholder="0812xxxxxxxx">
                            ...
                            <div>
                                ...
                                <input type="text" name="no_wa" value="{{ old('no_wa') }}" class="w-full bg-white border border-slate-200 bg-white border-slate-300 rounded-lg px-3 py-2.5 text-slate-800 text-sm focus:border-sky-500 focus:ring-sky-500" placeholder="0812xxxxxxxx">
                            </div>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-500 mb-1">Email Aktif <span class="text-rose-400">*</span></label>
                            <input type="email" name="email" value="{{ old('email') }}" class="w-full bg-white border border-slate-200 bg-white border-slate-300 rounded-lg px-3 py-2.5 text-slate-800 text-sm focus:border-sky-500 focus:ring-sky-500" required placeholder="contoh@email.com">
                        ...
                        <div>
                            ...
                            <textarea name="alamat" id="alamat_ktp" rows="2" class="w-full bg-white border border-slate-200 bg-white border-slate-300 rounded-lg px-3 py-2.5 text-slate-800 text-sm focus:border-sky-500 focus:ring-sky-500" required>{{ old('alamat') }}</textarea>
                        </div>
                        <div>
                            ...
                            <textarea name="alamat_domisili" id="alamat_domisili" rows="2" class="w-full bg-white border border-slate-200 bg-white border-slate-300 rounded-lg px-3 py-2.5 text-slate-800 text-sm focus:border-sky-500 focus:ring-sky-500">{{ old('alamat_domisili') }}</textarea>
                        </div>
                    </div>
                </div>
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select id="select_bank" name="nama_bank" class="w-full bg-white border border-slate-200 bg-white border-slate-300 rounded-lg px-3 py-2.5 text-slate-800 text-sm focus:border-sky-500 focus:ring-sky-500" required onchange="checkBank(this.value)">
                                <option value="" disabled {{ !old('nama_bank') ? 'selected' : '' }} class="bg-white">Pilih Bank...</option>
                                @foreach($banks as $bank)
                                <option value="{{ $bank }}" {{ old('nama_bank') == $bank ? 'selected' : '' }} class="bg-white">{{ $bank }}</option>
                                @endforeach
                                <option value="Lainnya" {{ old('nama_bank') == 'Lainnya' || (old('nama_bank') && !in_array(old('nama_bank'), $banks)) ? 'selected' : '' }} class="bg-white">Lainnya</option>
                            </select>
                        </div>
                        <div id="manual_bank_container" class="{{ old('nama_bank') == 'Lainnya' || (old('nama_bank') && !in_array(old('nama_bank'), $banks)) ? '' : 'hidden' }}">
                            ...
                            <input type="text" id="manual_bank" name="manual_bank" value="{{ in_array(old('nama_bank'), $banks) ? '' : old('nama_bank') }}" class="w-full bg-white border border-slate-200 bg-white border-slate-300 rounded-lg px-3 py-2.5 text-slate-800 text-sm focus:border-sky-500 focus:ring-sky-500">
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-medium text-slate-500 mb-1">Nomor Rekening <span class="text-rose-400">*</span></label>
                                <input type="text" name="no_rekening" value="{{ old('no_rekening') }}" class="w-full bg-white border border-slate-200 bg-white border-slate-300 rounded-lg px-3 py-2.5 text-slate-800 text-sm focus:border-sky-500 focus:ring-sky-500" required>
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-500 mb-1">Nama Pemilik <span class="text-rose-400">*</span></label>
                                <input type="text" name="nama_pemilik_rekening" value="{{ old('nama_pemilik_rekening') }}" class="w-full bg-white border border-slate-200 bg-white border-slate-300 rounded-lg px-3 py-2.5 text-slate-800 text-sm focus:border-sky-500 focus:ring-sky-500" required>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="flex justify-end gap-3 mt-6">
            ...
            <button type="submit" class="bg-[#cf9e50] hover:bg-[#b48842] text-white font-semibold py-2 px-4 rounded-xl shadow-sm transition-all px-8 py-3 rounded-xl text-sm font-semibold">Simpan Nasabah</button>
        </div>
    </form>
</div>
<script>
