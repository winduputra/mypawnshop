<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-8">
            <h3 class="text-xl font-bold text-slate-800 mb-6">Buat Akun Admin Baru</h3>
            <form action="{{ route('admin.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" class="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-slate-800 focus:border-sky-500 focus:ring-sky-500" required>
                    @error('name') <p class="text-rose-400 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-500 mb-2">Email</label>
                    <input type="email" name="email" value="{{ old('email') }}" class="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-slate-800 focus:border-sky-500 focus:ring-sky-500" required>
                ...
                <div>
                    ...
                    <input type="password" name="password" class="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-slate-800 focus:border-sky-500 focus:ring-sky-500" required>
                ...
                <div>
                    ...
                    <input type="password" name="password_confirmation" class="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-slate-800 focus:border-sky-500 focus:ring-sky-500" required>
                </div>
                <button type="submit" class="bg-[#cf9e50] hover:bg-[#b48842] text-white font-semibold rounded-xl shadow-sm transition-all w-full py-4 text-lg">Buat Akun Admin</button>
            </form>
        </div>
    </div>
    @endsection
